...
export default function WritePostPage({
  ...
  const handleVideoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("video/")) {
      setError("You can only upload video files.");
      return;
    }

    // 30초 제한 체크 (실제로는 서버에서 처리)
    setVideo(file);
    setError(null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to the board page after successful submission
      router.push(`/${category}/${subCategory}/${board}`);
    } catch (err) {
